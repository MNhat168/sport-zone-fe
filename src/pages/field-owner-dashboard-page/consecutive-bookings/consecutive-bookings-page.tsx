...
export default function ConsecutiveBookingsPage() {
    ...
    useEffect(() => {
        try {
            const stored = localStorage.getItem('ownerHiddenConsecutiveBookingIds');
            if (stored) {
    ...
    useEffect(() => {
        try {
            localStorage.setItem('ownerHiddenConsecutiveBookingIds', JSON.stringify(hiddenIds));
        } catch (error) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('consecutive-booking-tab-change', handleTabChange as EventListener);
        return () => {
            window.removeEventListener('consecutive-booking-tab-change', handleTabChange as EventListener);
        };
    }, [updateSearchParams]);
                ...
                <Card className="bg-white shadow-md rounded-none border-0 gap-0">
                    <CardHeader className="border-b">
                        <CardTitle className="text-xl font-semibold text-start">
                            Đặt Sân Nhiều Ngày Liên Tiếp
                        </CardTitle>
                        <p className="text-gray-600 text-base mt-1.5 text-start">
                            Quản lý và theo dõi tất cả các đặt sân nhiều ngày liên tiếp của bạn (ví dụ: đặt từ thứ 2 đến thứ 6 cùng một khung giờ).
                        </p>
                        <div className="mt-4">
                            <BookingFilters
                                    ...
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-gray-400 text-sm mt-2">
                                            {searchQuery || timeFilter !== "all" || activeTab !== 'confirmed'
                                                ...
                                                : "Bạn chưa có đặt sân nhiều ngày liên tiếp nào"}
                                        </p>
                                    </div>
                                ) : (
                                    <BookingTable
