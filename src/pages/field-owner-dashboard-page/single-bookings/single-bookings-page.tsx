...
export default function SingleBookingsPage() {
    ...
    // Default to 'confirmed' - no pending tab for single bookings
    ...
    // Helper to fetch bookings - using 'status' instead of 'transactionStatus'
    const fetchBookings = useCallback((isInitial = false) => {
        ...
        dispatch(
            getMyFieldsBookings({
                ...
                page: currentPage,
                limit: itemsPerPage,
                type: 'field', // Only single field bookings
                recurringFilter: 'none', // Chỉ lấy single bookings (không có recurringGroupId)
                sortBy,
                sortOrder,
            })
        ).then(() => {
