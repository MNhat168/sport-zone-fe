...
// Booking Status type for coach bookings
...
export default function CoachBookingsPage() {
    ...
    // Helper to fetch bookings - coach-only bookings
    const fetchBookings = useCallback((isInitial = false) => {
        ...
        dispatch(
            getMyBookings({
                status: activeTab,
                type: 'coach', // Coach-only bookings
                startDate,
                endDate,
                page: currentPage,
                limit: itemsPerPage,
    ...
    return (
        <>
            ...
            <AlertDialog open={confirmState.open} onOpenChange={(open) => !open && setConfirmState(prev => ({ ...prev, open: false }))}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Xác nhận hủy đặt HLV</AlertDialogTitle>
                        <AlertDialogDescription className="space-y-2">
                            <p>Bạn có chắc chắn muốn hủy đặt HLV này không? Hành động này không thể hoàn tác.</p>
                            <p className="text-sm text-yellow-600 font-medium bg-yellow-50 p-3 rounded-md border border-yellow-200">
                                Lưu ý: Bạn chỉ được hoàn tiền từ huấn luyện viên khi hủy đặt trước 12h tính từ thời điểm bắt đầu slot bạn đặt.
                            </p>
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
