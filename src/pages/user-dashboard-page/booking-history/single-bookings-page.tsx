...
// Booking Status type for single bookings
...
export default function SingleBookingsPage() {
    ...
    // Use TanStack Query for data fetching (automatic background refetch every 30s!)
    const { data: response, isLoading, error } = useUserBookings({
        status: activeTab,
        type: 'field',
        recurringFilter: 'none', // Only single bookings
        startDate,
        endDate,
        page: currentPage,
        limit: itemsPerPage,
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (action === 'cancel') {
                ...
                // No manual refresh needed! TanStack Query auto-invalidates cache
            }
        } catch (e) {
                    ...
                    <div className="space-y-4 md:space-y-6">
                        <Card className="bg-white shadow-md rounded-none border-0 gap-0">
                            <CardHeader className="border-b p-4 md:p-6">
                                <CardTitle className="text-lg md:text-xl font-semibold text-start">
                                    Đặt Sân Đơn Lẻ
                                </CardTitle>
                                <p className="text-gray-600 text-sm md:text-base mt-1.5 text-start">
                                    Quản lý và theo dõi tất cả các đặt sân đơn lẻ của bạn.
                                </p>
                                ...
                                <div className="flex flex-wrap gap-1 md:gap-1 mt-4">
                                    ...
                                    <Button
                                        ...
                                        className={`text-xs md:text-sm px-3 md:px-4 py-2 ${activeTab === "completed" ? "bg-black text-white hover:bg-gray-800" : "hover:bg-gray-100"
                                            } transition-all duration-200`}
                                    >
                                        Hoàn thành
                                    </Button>
                                            ...
                                            <div className="text-center py-12">
                                                ...
                                                <p className="text-gray-400 text-sm mt-2">
                                                    {searchQuery || timeFilter !== "all" || activeTab !== 'confirmed'
                                                        ...
                                                        : "Bạn chưa có đặt sân đơn lẻ nào"}
                                                </p>
                                            </div>
                                        ) : (
                                            <BookingTable
