...
export default function ForgotPassword() {
  ...
  const res = await axiosPublic.post("/auth/forgot-password", { email });
      ...
      setAlertMessage(res.data?.message || `Verification code sent to ${email}`);
      setShowAlert(true);
  ...
  const res = await axiosPublic.post("/auth/verify", { email, verificationToken: code });
      ...
      navigate("/auth/reset-password", { state: { email, resetPasswordToken: code } });
    } catch (err: any) {
      setIsLoading(false);
      setAlertType("danger");
      setAlertMessage(err.response?.data?.message || "Invalid code");
      setShowAlert(true);
      setTimeout(() => setShowAlert(false), 3000);
    }
  };
  ...
  return (
    <div className="sports-login-fullwidth">
                ...
                {step === "email" ? (
                  <>
                    ...
                    <div className="text-center">
                      <Button variant="link" className="back-link p-0" onClick={() => navigate("/login")}>
      ...
      <style >{`
        ...
        .forgot-password-icon .text-primary {
