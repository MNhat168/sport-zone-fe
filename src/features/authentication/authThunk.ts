...
export const signInWithEmailAndPassword = createAsyncThunk<
    Pick<AuthResponse, "user"> & { authMethod: 'bearer' },
    LoginPayload,
    { rejectValue: ErrorResponse }
>("auth/login", async (payload, thunkAPI) => {
    try {
        // Always use Bearer token flow
...
>("auth/google", async (payload, thunkAPI) => {
    try {
        // Always use Bearer token flow
        logger.debug("Using Bearer token Google authentication");
        ...
        sessionStorage.setItem('auth_access_token', raw.accessToken);
        sessionStorage.setItem('auth_refresh_token', raw.refreshToken);
        logger.debug("Google login tokens stored in sessionStorage");
