...
class WebSocketService {
  ...
  connect() {
    ...
    // Get user data from storage (sessionStorage -> localStorage)
    ...
    try {
      ...
      logger.error("[WebSocketService] Failed to parse user from storage", e);
    }

    if (!userId) {
      logger.error("[WebSocketService] No user ID found in storage");
      return;
    }
